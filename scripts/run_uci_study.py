...
from responsible_aied_evaluation.core import (
    ...
    threshold_sensitivity,bootstrap_interval,
)
...
def main():
    ...
    result={
        ...
        "bootstrap_uncertainty":{
            metric:bootstrap_interval(records,metric,n_resamples=1000,seed=SEED)
            for metric in ["accuracy","selection_rate_gap","tpr_gap","fpr_gap"]
        },
    ...
    lines=[
        ...
        "- DOI: 10.24432/C5MC89",
        f"- n: {len(X)}; train: {len(train_idx)}; test: {len(test_idx)}",
        f"- Predictors after timing/protected-feature exclusions: {model_X.shape[1]}",
        f"- Excluded post-enrollment columns: {len(forbidden)}",
        f"- Protected/group columns excluded from prediction: {', '.join(protected)}","",
        "## Overall holdout performance","",
        f"- ROC-AUC: {result['overall']['roc_auc']:.4f}",
        f"- Brier score: {result['overall']['brier']:.4f}",
        f"- ECE-10: {result['overall']['ece_10']:.4f}","",
        "## Gender-group audit at threshold 0.50","",
        f"- Audit status: {audit.get('status')}",
        f"- Selection-rate gap: {audit.get('selection_rate_gap')}",
        f"- TPR gap: {audit.get('tpr_gap')}",
        f"- FPR gap: {audit.get('fpr_gap')}",
        f"- Equalized-odds-style gap: {audit.get('equalized_odds_gap')}","",
